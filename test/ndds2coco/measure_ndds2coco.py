...
intermediate_dataset_output_path = 'measure_total_test.json'

if not file_exists(intermediate_dataset_output_path):
    # Load NDDS Dataset
    ...
    for frame in ndds_dataset.frames:
        ...
        for ann_obj in frame.ndds_ann.objects:
            ...
            elif ann_obj.class_name == 'mark_10th_place':
                obj_type, obj_name = target_obj_type, '10s-rectangles'
                ann_obj.class_name = f'{obj_type}_{obj_name}'
            elif ann_obj.class_name == 'marking_bottom':
                obj_type, obj_name = target_obj_type, 'ticks-bottom'
                ann_obj.class_name = f'{obj_type}_{obj_name}'
            elif ann_obj.class_name == 'marking_top':
                obj_type, obj_name = target_obj_type, 'ticks-top'
                ann_obj.class_name = f'{obj_type}_{obj_name}'
            elif ann_obj.class_name == 'hook':
                obj_type, obj_name = target_obj_type, 'hook'
                ann_obj.class_name = f'{obj_type}_{obj_name}'
    ...
    dataset = Measure_COCO_Dataset.from_ndds(
        ...
        categories=COCO_Category_Handler.load_from_path('/home/clayton/workspace/prj/data_keep/data/ndds/categories/measure_all_adv.json'),
        ...
        visibility_threshold_dict={'measure': 0.01, '10s-rectangles': -1, 'ticks-bottom': -1, 'ticks-top': -1, 'hook': -1},
        allow_unfound_seg=True
    )

    dataset.save_to_path(save_path=intermediate_dataset_output_path)
    dataset.display_preview(show_details=True)
else:
    dataset = Measure_COCO_Dataset.load_from_path(json_path=intermediate_dataset_output_path)
